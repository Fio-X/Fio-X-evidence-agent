...
function writePhaseSurfaceSnapshot() {
  try {
    const root = artifactRoot(); if (!root) return;
    const phase=process.env.NEWSROOM_PHASE??"all", profile=process.env.NEWSROOM_TOOL_PROFILE??"investigate";
    const configured=toolSurfaceForProfile(profile,phase);
    const snapshot={schema_version:"0.1.0",phase,tool_profile:profile,profile_tool_count:configured.profile_tool_count,configured_phase_tool_count:configured.effective_phase_tool_count,effective_phase_tool_count:registeredPhaseTools.length,effective_phase_schema_bytes:registeredPhaseTools.reduce((sum,tool)=>sum+tool.schema_bytes,0),schema_byte_method:"utf8-json-parameters-estimate",registered_tool_names:registeredPhaseTools.map(tool=>tool.name)};
    const path=join(root,"runtime","phase-tool-surface.json"); mkdirSync(dirname(path),{recursive:true}); writeFileSync(path,JSON.stringify(snapshot,null,2)+"\n","utf8");
  } catch { /* observability must not alter registration or evidence behavior */ }
}
...
function registerScopedTool(pi: ExtensionAPI, tool: any) {
  ...
  registeredPhaseTools.push({name, schema_bytes:(()=>{try{return Buffer.byteLength(JSON.stringify(tool?.parameters??{})??"","utf8")}catch{return 0}})()});
