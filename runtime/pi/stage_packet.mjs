...
function packetPayload(packet) {
  return {
    ...
    required_visual_modes: packet.required_visual_modes,
  };
}
...
function assertPacketShape(packet) {
  if (!packet || packet.schema_version !== STAGE_PACKET_SCHEMA_VERSION) {
    throw fail('retry_packet.schema_version', `schema_version must be ${STAGE_PACKET_SCHEMA_VERSION}`);
  }
  if (typeof packet.goal !== 'string' || packet.goal.length === 0) throw fail('retry_packet.goal', 'goal is required');
  for (const key of ['refs', 'hashes', 'passed_gates', 'remaining_gaps', 'required_visual_modes']) {
    if (!packet[key] || typeof packet[key] !== 'object') throw fail('retry_packet.field_required', `${key} is required`);
  }
  for (const key of ['passed_gates', 'remaining_gaps', 'required_visual_modes']) {
    if (!Array.isArray(packet[key]) || !packet[key].every((value) => typeof value === 'string')) {
      throw fail('retry_packet.array', `${key} must be a string array`);
    }
  }
...
export function createRetryContextPacket({ goal, refs, hashes, passed_gates, remaining_gaps, required_visual_modes }) {
  const packet = {
    ...
    refs: { ...refs },
    hashes: { ...hashes },
    passed_gates: [...new Set(passed_gates ?? [])].sort(),
    remaining_gaps: [...new Set(remaining_gaps ?? [])].sort(),
    required_visual_modes: [...new Set(required_visual_modes ?? [])].sort(),
  };
...
export async function resolveRetryContextPacket(packet, { root, read = readFile } = {}) {
  assertPacketShape(packet);
  if (typeof root !== 'string' || !root) throw fail('retry_packet.root', 'root is required');
  const resolved = {};
  for (const kind of Object.keys(packet.refs)) {
    const absolute = path.resolve(root, packet.refs[kind]);
    ...
    let bytes;
    try { bytes = await read(absolute); } catch { throw fail('retry_packet.missing_ref', `referenced artifact is unavailable: ${kind}`); }
...
export function validateRetryContextPacket(packet) {
  try { assertPacketShape(packet); return { passed: true, issues: [] }; }
  catch (error) { return { passed: false, issues: [{ code: error.code ?? 'retry_packet.invalid', message: error.message }] }; }
}
