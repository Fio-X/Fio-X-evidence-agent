...
---
Use Plotly when the reader benefits from hover, selection, responsive browser layout, linked views, qualified uncertainty bands, scientific spatial views, Sankey flow inspection or standard interactive statistical graphics. In `verification_mode="verified"`, keep figure data derived only from verified rows. In `verification_mode="draft"`, supplied rows may be explored, but the self-contained HTML/SVG must be labeled DRAFT and cannot enter a publishable publication or infographic.
