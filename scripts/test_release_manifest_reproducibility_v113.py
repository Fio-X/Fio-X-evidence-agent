...
assert all(not x['path'].startswith('target/') for x in b['source_tree']['files'])
assert all(x['path'] != '.env' and not x['path'].startswith(('logs/', 'codex-session-')) for x in b['source_tree']['files'])
