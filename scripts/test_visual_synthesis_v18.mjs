...
const unboundCausal={...spec,dek:'The observed change was crisis-driven.',visual_thesis:'Conflict caused the measured shift.'};
const nonCausalRecords=verified.map(claim_id=>({claim_id,claim_kind:'quantitative',verification:{authority:'system',publishable:true}}));
const causalLint=lintInfographicSpec(unboundCausal,assets,{...context,verified_claim_records:nonCausalRecords});
assert.equal(causalLint.passed,false);
assert.ok(causalLint.blockers.some(x=>x==='unbound_causal_editorial_language:dek'));
assert.ok(causalLint.blockers.some(x=>x==='unbound_causal_editorial_language:visual_thesis'));
