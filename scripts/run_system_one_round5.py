#!/usr/bin/env python3
"""Run isolated System One round-5 token-budget integration experiments."""
...
MANIFEST=ROOT/"experiments"/"system-one"/"round5-manifest.json"
...
def execute(exp, baseline, model, tmp):
    eid=exp["id"]; remote_branch=f"exp/system-one-{eid}"
    wt=tmp/eid; log=tmp/f"{eid}.log"
    result_rel=Path("experiments/system-one/results")/f"{eid}.md"
    allowed=set(exp.get("allowed_files",[]))|{str(result_rel)}
    state={"id":eid,"branch":remote_branch,"state":"STARTED","log":str(log)}
    print(f"{eid}: STARTED", flush=True)
    try:
        run(["git","worktree","add","--detach",str(wt),baseline])
        prompt=(
          "Read AGENTS.md, experiments/system-one/round4-summary.md, round5-plan.md, and round5-manifest.json. "
          "Execute exactly experiment "+eid+". "
          "Inspect only the listed origin/exp input branches as implementation evidence using git show/diff. "
          f"Modify only its allowed_files and {result_rel}. "
          "Preserve all invariants. Implement the smallest bounded opt-in or measurement-only A/B variant. "
          "Run common checks plus focused tests and the requested repeated measurements. "
          "Write a concise evidence-backed result and end with exactly one classification: PROMOTE, HOLD, REJECT, or INCONCLUSIVE."
        )
...
def main():
    ...
    tmp=Path(tempfile.mkdtemp(prefix="fiox-system-one-r5-"))
    ...
    with ThreadPoolExecutor(max_workers=a.jobs) as pool:
        ...
        for f in as_completed(fs):
            r=f.result(); results.append(r); print(f"{r['id']}: {r['state']}", flush=True)
