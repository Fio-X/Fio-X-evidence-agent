...
with tempfile.TemporaryDirectory() as td:
 ...
 fam=json.loads(out.read_text())['story_families']['local_map']; assert fam['status']=='QUALIFIED'; p=fam['probabilities']; assert p['qgis_cartography']>p['python_publication']>p['r_editorial']
 # default threshold should keep this small sample pending
 pending=Path(td)/'pending.json'; subprocess.run([sys.executable,str(root/'scripts/build_backend_priors.py'),str(inp),'--output',str(pending)],check=True,capture_output=True,text=True)
 assert json.loads(pending.read_text())['story_families']['local_map']['status']=='PENDING'
print('backend priors v1.29: PASS')
