...
def first_numeric(metrics: dict, needles: tuple[str,...]):
    for key,value in metrics.items():
        if isinstance(value,(int,float)) and all(token in key.lower() for token in needles):
            return value
    return None
...
def trial_row(index: int, exit_code: int, artifact: Path|None, qual: dict, wall_ms: int):
    ...
    passed=exit_code==0 and qual.get('status')=='PASS'
    return {
        'trial': index,
        'exit_code': exit_code,
        'passed': passed,
...
def reliability_summary(rows: list[dict]):
    ...
    def stats(key):
        vals=numeric(key)
        return {'total':sum(vals),'mean':sum(vals)/len(vals)} if vals else {'total':None,'mean':None}
    return {
...
def main():
    ...
    if a.qualification_files:
        ...
        if a.trials < 1: raise SystemExit('--trials must be >= 1')
