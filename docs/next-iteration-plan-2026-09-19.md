...
当前状态：迭代 1 已在本地完成；迭代 2 的新回归组已在本地通过并接入 CI，等待首次 GitHub Actions 确认。公开提交 Lieflat 非商业许可代码已获得项目方确认，但其子树仍保持独立许可边界。
...
1. 将当前变更提交到功能分支并推送，触发首次完整 CI。
2. 对 CI 失败只做确定性修复；不在本轮扩展 Linux 专项或新增图形类型。
3. CI 全绿后启动浏览器矩阵截图验收，重点检查空白画布、响应式排版、标签碰撞和离线请求。
4. 把截图、测试摘要与第三方许可说明附到 PR，再进入代码审查。
